  ...
  useEffect(() => {
    if (modalData) {
      ...
      modalData.uploaded_files.map((element, index) => {
        axios
          .get("api/get-uploaded-screenshot/" + element.uploaded_files_id, {
            responseType: "arraybuffer",
          })
          .then(
            (res) => {
              const base64Str = Buffer.from(res.data, "utf8").toString(
                "base64"
              );
      ...
      <Table className="table mb-0">
        <tbody>
          <tr>
            ...
            <td>
              {moment(modalData.created_at).format("MMMM D, YYYY h:mm a")}
            </td>
          </tr>
          <tr>
              ...
              <td>
                {modalData.request_assignment.map((person) => (
                  <span
                    ...
                    key={person.request_assignment_id + "request_assignment"}
                  >
                    <Badge>
                      {person.tbl_users.first_name +
                        " " +
                        (person.tbl_users.middle_name
                          ? person.tbl_users.middle_name.charAt(0) + ". "
                          : "") +
                        person.tbl_users.last_name +
                        (person.tbl_users.suffix
                          ? ", " + person.tbl_users.suffix
                          : "")}
                    </Badge>{" "}
                    <RemovePersonnel
                      personnelId={person.assigned_personnel}
                      requestId={modalData.request_details_id}
          ...
          <tr>
            ...
            <td>
              {modalData.tbl_users.first_name +
                " " +
                (modalData.tbl_users.middle_name
                  ? modalData.tbl_users.middle_name.charAt(0) + ". "
                  : "") +
                modalData.tbl_users.last_name +
                (modalData.tbl_users.suffix
                  ? ", " + modalData.tbl_users.suffix
                  : "")}
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              {
                modalData.service_request.service_request_types
                  .service_request_type_description
              }
            </td>
          </tr>
          <tr>
