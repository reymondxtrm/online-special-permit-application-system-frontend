...
const AdminTable = ({ applicationType, status, activeTab }) => {
  ...
  const dateOfEvent = (from, to) => {
    if (!!to || !!from) {
      if (to === from) {
        return formateDateIntoString(from);
      }
      return `${formateDateIntoString(from)} to ${formateDateIntoString(to)}`;
    }
    return "";
  };
                            ...
                            >
                              <DropdownItem
                                onClick={() => {
                                  ...
                                  // Proceed with creating the order of payment
                                  // Swal.fire({
                                  //   title: "Success!",
                                  //   text: "Order of Payment is being created.",
                                  //   icon: "success",
                                  //   confirmButtonText: "Okay",
                                  // }).then(() => {
